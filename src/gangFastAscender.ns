...
function getMoney(ns) {
  return ns.getServerMoneyAvailable('home') - 100000000
}
  ...
  if (getMoney(ns) > 0) {
    ...
    for (let i = 0; i < gangMemberNames.length; i++) {
      ...
      while (ascended) {
        ...
        ns.print(`${gangMemberInfo.str_asc_mult}/${strengthAscensionMultHardLimit}`)
        if (gangMemberInfo.str_asc_mult < strengthAscensionMultHardLimit) {
          let hasBat = gangMemberInfo.upgrades.includes(baseballBat)

          if (!hasBat && getMoney(ns) > 2000000) {
            hasBat = ns.gang.purchaseEquipment(gangMemberName, baseballBat)
          }

          if (hasBat) {
            //ns.print(ascensionResult)

            if (gangMemberInfo.str_asc_mult < strengthAscensionMultHardLimit / 3) {
              function checkAverageImprovment(ns) {
                const ascensionResult = ns.gang.getAscensionResult(gangMemberName)
                if (ascensionResult == null || ascensionResult["str"] == null) {
                  return 1
                }
                return (ascensionResult["str"] + ascensionResult["def"] + ascensionResult["dex"]) / 3
              }

              while (averageImprovment < 1.1) {
                averageImprovment = checkAverageImprovment(ns)
                ns.print(`[${localeHHMMSS()}] Ascention average improvment : ${ns.nFormat(averageImprovment, "0.0000%")}`)
                await ns.sleep(1500)
              }
    ...
    const minimumReputationToBuyDex = 50000
          ...
          if (gangMemberInfo.dex_asc_mult < strengthAscensionMultHardLimit) {
            ...
            if (!missingEq.length) {
              if (gangMemberInfo.dex_asc_mult < strengthAscensionMultHardLimit) {
                ...
                while (averageImprovment < 1.01) {
                  averageImprovment = checkAverageImprovment(ns)
                  ns.print(`[${localeHHMMSS()}] Ascention average improvment : ${ns.nFormat(averageImprovment-1, "0.0000%")}`)
                  await ns.sleep(1500)
                }
                ns.gang.ascendMember(gangMemberName)
                ascended = true
